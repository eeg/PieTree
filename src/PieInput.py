...
def ParseInput():
    ...
    if treefile:
        ...
        if not root:
            ...
            PieReadTree.AssignNodeTimes(root)
            age = root.Age()
    else:
        root = None
...
def AddParserArgs(parser, nstates, format_choices):
    ...
    parser.add_argument("--scalebar", \
            help="display the temporal scale [yes, no, number]")
...
def SetDefaults(c, ntips, age):
    ...
    if c.scalebar == "no":
        ...
        sb = {}
        sb["textsize"] = max([c.tipnamesize, c.nodenamesize, c.boxsize, c.pieradius])
        c.ymargin += sb["textsize"] * 2
        if c.scalebar == "yes":
            sb["length"] = age/4.
        else:
            sb["length"] = float(c.scalebar)
        c.scalebar = sb
